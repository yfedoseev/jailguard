...
func TestDetectInjection(t *testing.T) {
	...
	for _, text := range cases {
		t.Run(text[:min(40, len(text))], func(t *testing.T) {
			...
			// per-platform float rounding. IsInjection==true + Score>0.5
			// asserts above already confirm it's classified as an attack.
			if r.Risk != jailguard.RiskMedium && r.Risk != jailguard.RiskHigh && r.Risk != jailguard.RiskCritical {
				t.Errorf("Detect(%q): expected Medium/High/Critical risk, got %v", text, r.Risk)
			}
		})
	}
}
