...
3-second warm-up phase. This represents **throughput-mode latency** — the
ONNX session is already loaded and warmed up, equivalent to a long-running
server process handling its second and subsequent requests.
...
and MLP forward pass. This is the **end-to-end per-request cost** a
production server would observe on steady-state traffic.
