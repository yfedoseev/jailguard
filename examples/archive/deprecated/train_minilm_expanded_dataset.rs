//! Train JailGuard on EXPANDED dataset with 15,185 combined samples.
//!
//! This example trains on the expanded dataset combining:
//! - deepset/prompt-injections (662 samples)
//! - TrustAIRLab In-The-Wild (14,523 samples)
//! Total: 15,185 samples (23x expansion)
//!
//! Expected improvements over baseline (78.9% on 662 samples):
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    println!("\n╔════════════════════════════════════════════════════════════════════╗");
    println!("║ JailGuard SOTA: Training on Expanded Dataset (15,185 samples)      ║");
    println!("║    23x larger than baseline - Expecting 82-87% accuracy            ║");
    println!("╚════════════════════════════════════════════════════════════════════╝\n");

    // Load embeddings
    println!("📥 Loading Pre-computed Embeddings...");
    let embedding_path = "data/combined_minilm_embeddings.json";
    if !Path::new(embedding_path).exists() {
        println!("⚠️  Embeddings not found. Generating with:");
        println!("    python3 scripts/precompute_embeddings_minilm.py --data data/combined_injection_dataset.json --output {}", embedding_path);
        return Ok(());
    }
    ...
    println!("   Optimizer: SGD with learning rate 0.01");
    println!("{}", "═".repeat(70));

    let mut classifier = TrainableClassifier::new(384, 256, 0.01);
    let num_epochs = 50;
    ...
    println!("{}", "═".repeat(70));
    println!(
        "Dataset Size:    662 (baseline)  →  {} (expanded, 23x)",
        loader.len()
    );
    println!("Training Samples: 397            →  {} (23x)", train.len());
    println!(
    ...
    if test_acc >= 0.80 {
        println!(
            "║ ✅ SUCCESS: {} accuracy (target: 80%+)                            ║",
            format!("{:.1}%", test_acc * 100.0)
        );
    } else {
        println!(
            "║ ⚠️  Accuracy: {:.1}% (target: 80%+)                              ║",
            test_acc * 100.0
        );
    }
    println!("╚════════════════════════════════════════════════════════════════════╝\n");
